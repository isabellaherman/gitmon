...
export default function StreakDisplay({ streak }: StreakDisplayProps) {
  // Determine fire color based on streak level
  const getFireColor = (streakCount: number) => {
    if (streakCount === 0) return "text-gray-400";
    if (streakCount <= 3) return "text-orange-500";
    if (streakCount <= 7) return "text-orange-600";
    if (streakCount <= 15) return "text-red-500";
    if (streakCount <= 30) return "text-red-600";
    return "text-blue-500"; // Legendary streak
  };
  ...
  return (
    <div className="flex items-center justify-center gap-2 p-2">
      ...
      <svg
        ...
        viewBox="0 0 24 24"
        fill="currentColor"
        className={`${getFireColor(streak)} transition-colors duration-300`}
      >
