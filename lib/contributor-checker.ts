...
async function checkGitMonContributions(githubUsername: string): Promise<boolean> {
  try {
    ...
    if (contributorsResponse.ok) {
      ...
      const isInContributors = contributors.some((contributor: { login: string }) =>
    ...
    if (prsResponse.ok) {
      ...
      const hasMergedPR = pullRequests.some((pr: { user: { login: string }, merged_at: string | null }) =>
    ...
    if (commitsResponse.ok) {
      ...
      const hasCommits = commits.some((commit: {
